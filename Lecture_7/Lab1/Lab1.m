...
% -------------------------------------------------------------------------
% Quick Notes:
% - Replaced default images with 'pout.tif' and 'rice.png'.
...
% *** MODIFICATION: Using 'pout.tif' and 'rice.png' as example replacements ***
if exist('rice.png','file')
    I_rgb = imread('rice.png');
elseif exist('pout.tif','file')
    I_rgb = repmat(imread('pout.tif'), 1, 1, 3); % fallback, convert to 3-channel
else
    % Fallback if neither 'rice.png' nor 'pout.tif' are found
    error('Neither "rice.png" nor "pout.tif" found. Please ensure image files are in the current directory or change the file names.');
end
figure; imshow(I_rgb); title('Original RGB (Loaded Product Image)');
...
figure; imshow(I); title('Grayscale Product Image');
